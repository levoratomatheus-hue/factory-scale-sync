...
export function useOrdens(date?: string) {
  ...
  const fetchOrdens = useCallback(async () => {
    ...
    let query = supabase.from("ordens").select("id, produto, lote, quantidade, status, posicao, balanca, linha, obs, marca, requer_mistura, orientacoes, formula_id, tamanho_batelada, data_programacao, hora_inicio, hora_fim, obs_linha, motivo_reprovacao, quantidade_real, temperaturas").order("posicao", { ascending: true, nullsFirst: false }).limit(500);
...
export function useHistorico(dataInicio?: string, dataFim?: string) {
  ...
  const fetchHistorico = useCallback(async () => {
    ...
    let query = supabase
      .from("ordens")
      .select("id, lote, produto, quantidade, hora_inicio, hora_fim, quantidade_real, linha, balanca, data_programacao, status, marca, data_conclusao, formula_id, obs, obs_laboratorio, obs_linha, motivo_reprovacao, temperaturas")
      .eq("status", "concluido")
