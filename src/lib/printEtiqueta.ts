...
export async function imprimirEtiqueta(data: EtiquetaData) {
  ...
  if (data.marca === "Pigma") {
    ...
    doc.text("PIGMA", W / 2, 10.5, { align: "center" });

    doc.setFontSize(4.5);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(157, 64, 120);
    doc.text("Pigmentos de Alta Performance", W / 2, 14, { align: "center" });
    ...
    doc.text(zanPart, startX, 10.5);
    doc.setTextColor(29, 78, 216);
    doc.text(colPart, startX + doc.getTextWidth(zanPart), 10.5);

    doc.setFontSize(4.5);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(99, 80, 180);
    doc.text("masterbatches", W / 2, 14, { align: "center" });
    ...
    doc.text(data.marca ?? "", W / 2, 10.5, { align: "center" });
  ...
  const prodY = headerH + 6;
  doc.text(prodLines, W / 2, prodY, { align: "center" });
  const prodBottom = prodY + (prodLines.length - 1) * 3.5;

  // ── Separador ────────────────────────────────────────────────────────────
  const sepY = prodBottom + 3.5;
  ...
  campos.forEach(({ label, value }, i) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(15, 15, 15);
    doc.text(value, cx, fieldsY + 4.5, { align: "center" });
  });
